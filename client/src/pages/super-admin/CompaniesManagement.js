...
const CompaniesManagement = () => {
  ...
  const confirmDeleteCompany = async () => {
    ...
    try {
      setDeleting(true);
      const response = await fetch(`${API_URL}/api/super-admin/companies/${companyToDelete.id}`, {
        method: 'DELETE',
        headers: {
          'Authorization': `Bearer ${localStorage.getItem('token')}`
        }
  ...
  const handleSubmitEdit = async (e) => {
    ...
    try {
      const response = await fetch(`${API_URL}/api/super-admin/companies/${editingCompany.id}`, {
        method: 'PUT',
        headers: {
          'Content-Type': 'application/json',
          'Authorization': `Bearer ${localStorage.getItem('token')}`
          ...
          <div className="relative top-20 mx-auto p-5 border w-96 shadow-lg rounded-md bg-white">
            <div className="mt-3">
              ...
              <form onSubmit={handleSubmitEdit} className="space-y-4">
                ...
                <div>
                  <label htmlFor="edit-domain" className="block text-sm font-medium text-gray-700">
                    Domain
                  </label>
                  <input
                    type="text"
                ...
                <div>
                  <label htmlFor="edit-subscriptionPlan" className="block text-sm font-medium text-gray-700">
                    Subscription Plan
                  </label>
                  <select
      ...
      {showDeleteModal && companyToDelete && (
        <div className="fixed inset-0 bg-gray-600 bg-opacity-50 overflow-y-auto h-full w-full z-50">
          <div className="relative top-20 mx-auto p-5 border w-96 shadow-lg rounded-md bg-white">
            <div className="mt-3">
              <div className="flex items-center justify-between mb-4">
                <h3 className="text-lg font-medium text-red-900">Delete Company</h3>
                <button
              ...
              <div className="mb-6">
                ...
                <p className="text-sm text-gray-600 mb-4">
                  This action will permanently deactivate <strong>{companyToDelete.name}</strong> and all associated data.
                </p>
                
                <div className="bg-gray-50 rounded-lg p-3 text-sm text-gray-700">
                  <p><strong>Company:</strong> {companyToDelete.name}</p>
                  <p><strong>Email:</strong> {companyToDelete.email}</p>
                  <p><strong>Domain:</strong> {companyToDelete.domain || 'N/A'}</p>
                </div>
              </div>
