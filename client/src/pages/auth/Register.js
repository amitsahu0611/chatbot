...
const Register = () => {
  const { register: registerUser, isAuthenticated, loading, userRole } = useAuth();
  ...
  const onSubmit = async (data) => {
    ...
    try {
      ...
      await registerUser({
        ...
        role: 'company_admin', // Default role for new registrations
      });
    } catch (error) {
  ...
  return (
    <div className="min-h-screen flex items-center justify-center bg-gray-50 py-12 px-4 sm:px-6 lg:px-8">
      ...
      {/* Enhanced Chat Widget - Permanent on register page */}
      <ChatWidget 
        companyId={13} 
        widgetId="widget_13_register" 
      />
    </div>
  );
};
