...
app.options("*", (req, res) => {
  ...
  if (origin && req.url.includes("/api/widget/")) {
    logger.info(
      `Widget preflight request from domain: ${origin} to ${req.url}`
    );
  }
  ...
  res.setHeader(
    ...
    "Content-Type, Authorization, X-Requested-With, Accept, Origin, Access-Control-Request-Method, Access-Control-Request-Headers, X-Forwarded-For, User-Agent, Referer"
  );
...
// Widget static files with proper CORS
app.get("/api/widget/chat.js", (req, res) => {
...
app.get("/api/widget/chat-with-theme.js", (req, res) => {
  const widgetPath = path.join(
    __dirname,
    "../public/widget/chat-with-theme.js"
  );
...
app.use("/api/widget", (req, res, next) => {
  ...
  if (origin) {
    logger.info(
      `Widget request from domain: ${origin} to ${req.url} (Method: ${req.method})`
    );
  }
...
app.use(
  ...
  (req, res, next) => {
    logger.info(
      `Widget route accessed: ${req.method} ${req.url} from ${
